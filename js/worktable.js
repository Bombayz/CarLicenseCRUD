...
function setStatus(id,to){
    ...
    $.sweetModal.confirm('คุณแน่ใจหรือไม่ ?', function() {
        ...
        $.ajax({
            ...
            success: function(data){
                swal("เรียบร้อย", "ส่งงานเรียบร้อยแล้ว !", "success");
                getTable();
                    ...
                    "columns": [
                        ...
                        { "data": "car_note",
                            render: function ( data, type, row ) {
                                return '<a onclick="getUserData('+row['no']+')" data-toggle="modal" data-target="#myModal">'+'ส่งงาน'+'</a>'
                            }
                        },
...
function getUserData(obj){
    ...
    $.getJSON( "../allservice.php?action=selectUserData&number="+obj, function( data ) {
        ...
        $('#setStatus').attr( "onclick", 'setStatus('+$('#no').val()+',3)' );
